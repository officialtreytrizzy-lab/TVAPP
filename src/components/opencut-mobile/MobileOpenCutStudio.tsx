...
type Tool = 'trim' | 'split' | 'text' | 'captions' | 'stickers' | 'audio' | 'speed' | 'filters' | 'adjust' | 'transform' | 'canvas' | 'export';
...
const TEXT_PRESETS: Array<{ label: string; patch: Partial<OpenCutTextLayer> }> = [
  { label: 'Hook', patch: { text: 'NEW DROP', y: 82, size: 48, weight: 900, color: '#ffffff', background: true, shadow: true, uppercase: true } },
  { label: 'Subtitle', patch: { text: 'Tap to edit caption', y: 78, size: 34, weight: 800, color: '#ffffff', background: true, shadow: true } },
  { label: 'Title', patch: { text: 'MAIN CHARACTER', y: 18, size: 44, weight: 900, color: '#ffffff', background: false, shadow: true, uppercase: true } },
  { label: 'Lower third', patch: { text: '@treytrizzy', y: 88, size: 30, weight: 800, color: '#f5d0fe', background: true, shadow: true } },
];
...
const FIRST_RUN_STEPS = [
  ...
  { icon: Sparkles, title: 'Stack tools fast', body: 'Use filters, captions, stickers, speed, transforms, fades, and aspect presets.' },
  { icon: Download, title: 'Export for socials', body: 'Render a clean mobile cut without leaving the Safari editor.' },
];

const QUALITY_CHIPS = ['9:16 ready', 'Safe-area tuned', 'CapCut-style tools'];
...
export default function MobileOpenCutStudio() {
  ...
  const addText = () => createTextLayer();

  const addCaption = () => {
    createTextLayer({ text: 'Tap to edit caption', y: 82, size: 34, weight: 900, background: true, shadow: true });
    setTool('captions');
  };

  const addSticker = (emoji: string) => {
    createTextLayer({ text: emoji, y: 48, size: 72, weight: 900, background: false, shadow: true });
    setTool('stickers');
  };
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#050510]/85 pt-safe backdrop-blur-2xl">
        <div className="mx-auto flex h-16 max-w-[430px] items-center gap-3 px-4 px-safe">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-0.5 flex items-center gap-1.5 text-[11px] font-semibold text-white/50"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,0.9)]" /> iPhone Studio</div>
          </div>
      ...
      {!selectedClip ? (
        <main className="relative mx-auto flex min-h-[calc(100dvh-4rem)] max-w-[430px] flex-col px-4 px-safe pb-[calc(env(safe-area-inset-bottom)+1.5rem)] pt-5">
          <section className="relative overflow-hidden rounded-[38px] border border-white/10 bg-white/[0.07] p-5 shadow-[0_28px_80px_rgba(0,0,0,0.45)] backdrop-blur-2xl">
            ...
            <div className="relative">
              ...
              <p className="mt-4 max-w-sm text-[15px] leading-6 text-white/60">Import a video, then use filters, captions, stickers, speed, transforms, fades, and export presets from Safari.</p>
              ...
              <button onClick={() => fileRef.current?.click()} className="mt-7 flex min-h-14 w-full items-center justify-center gap-2 rounded-[22px] bg-white px-5 py-4 text-base font-black text-slate-950 shadow-[0_18px_50px_rgba(255,255,255,0.18)] active:scale-[0.985]"><Upload className="h-5 w-5" /> Import video</button>
            </div>
          </section>
        ...
        <main className="relative mx-auto max-w-[430px] px-3 px-safe pb-[calc(env(safe-area-inset-bottom)+6.2rem)] pt-3">
          <section className="relative overflow-hidden rounded-[34px] border border-white/10 bg-black/55 p-2 shadow-[0_30px_90px_rgba(0,0,0,0.5)] backdrop-blur-xl">
            ...
            <div className="mb-2 flex items-center justify-between px-2 pt-1"><div className="inline-flex items-center gap-2 rounded-full bg-white/[0.08] px-3 py-1.5 text-[11px] font-bold text-white/75 ring-1 ring-white/10"><Layers className="h-3.5 w-3.5 text-violet-200" /> {selectedClip.filterPreset || 'Preview'}</div><div className="rounded-full bg-black/40 px-3 py-1.5 font-mono text-[11px] text-white/60 ring-1 ring-white/10">{formatTime(current)}</div></div>
            <div className={`relative mx-auto max-h-[43dvh] overflow-hidden rounded-[28px] bg-black ${aspectClass(project.aspect, selectedClip)}`}>
              ...
              {project.textLayers.map((layer) => {
                ...
                return <button key={layer.id} onClick={() => { setSelectedTextId(layer.id); setTool('text'); }} className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-2xl px-3 py-1.5 text-center font-black leading-tight ring-1 ring-white/10 ${layer.background ? 'bg-black/60 backdrop-blur-md' : ''} ${layer.shadow ? 'drop-shadow-[0_8px_16px_rgba(0,0,0,0.75)]' : ''}`} style={{ left: `${layer.x}%`, top: `${layer.y}%`, color: layer.color, fontSize: `${Math.max(14, layer.size / 3.2)}px` }}>{text}</button>;
              })}
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 px-1 text-center text-[11px]"><div className="rounded-2xl bg-white/[0.055] px-2 py-2 ring-1 ring-white/10"><span className="block text-white/40">Size</span><span className="font-mono text-white/70">{selectedClip.width}×{selectedClip.height}</span></div><div className="rounded-2xl bg-white/[0.055] px-2 py-2 ring-1 ring-white/10"><span className="block text-white/40">Length</span><span className="font-mono text-white/70">{formatTime(clipDuration)}</span></div><div className="rounded-2xl bg-white/[0.055] px-2 py-2 ring-1 ring-white/10"><span className="block text-white/40">Aspect</span><span className="font-mono text-white/70">{project.aspect}</span></div></div>
          </section>
          ...
          <section className="mt-3 rounded-[28px] border border-white/10 bg-white/[0.06] p-3 shadow-[0_18px_55px_rgba(0,0,0,0.28)] backdrop-blur-xl">
            <div className="mb-2 flex items-center justify-between text-xs text-white/60"><span>{formatTime(selectedClip.start)}</span><span className="font-black text-white">Timeline</span><span>{formatTime(selectedClip.end)}</span></div>
            ...
            <div className="no-scrollbar mt-3 flex gap-2 overflow-x-auto pb-1">{project.clips.map((clip) => <button key={clip.id} onClick={() => setProject((p) => ({ ...p, selectedClipId: clip.id }))} className={`min-w-28 rounded-2xl px-3 py-2 text-left text-xs ring-1 active:scale-95 ${clip.id === selectedClip.id ? 'bg-white text-slate-950 ring-white' : 'bg-white/[0.055] text-white/60 ring-white/10'}`}><span className="block truncate font-black">{clip.name}</span><span className="font-mono text-[10px] opacity-70">{formatTime(clip.end - clip.start)}</span></button>)}</div>
          </section>
          ...
          <section className="mt-3 min-h-44 rounded-[28px] border border-white/10 bg-white/[0.065] p-4 shadow-[0_18px_55px_rgba(0,0,0,0.28)] backdrop-blur-xl">
            <div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><div className="grid h-9 w-9 place-items-center rounded-2xl bg-white/[0.08] ring-1 ring-white/10"><ActiveToolIcon className="h-4 w-4" /></div><div><h3 className="text-sm font-black">{activeTool.label}</h3><p className="text-[11px] text-white/40">CapCut-style mobile tool</p></div></div><button onClick={() => fileRef.current?.click()} className="rounded-full bg-white/[0.08] px-3 py-2 text-[11px] font-bold text-white/70 ring-1 ring-white/10 active:scale-95">Replace</button></div>

            {tool === 'trim' && <div className="space-y-4"><div className="rounded-2xl bg-black/25 p-3 ring-1 ring-white/10"><label className="block text-xs font-bold text-white/60">Start · {formatTime(selectedClip.start)}</label><input className="mt-1 w-full accent-violet-500" type="range" min={0} max={selectedClip.duration} step={0.01} value={selectedClip.start} onChange={(e) => updateClip({ start: clamp(Number(e.target.value), 0, selectedClip.end - 0.1) })} /></div><div className="rounded-2xl bg-black/25 p-3 ring-1 ring-white/10"><label className="block text-xs font-bold text-white/60">End · {formatTime(selectedClip.end)}</label><input className="mt-1 w-full accent-violet-500" type="range" min={0} max={selectedClip.duration} step={0.01} value={selectedClip.end} onChange={(e) => updateClip({ end: clamp(Number(e.target.value), selectedClip.start + 0.1, selectedClip.duration) })} /></div></div>}
            ...
            {tool === 'canvas' && <div className="space-y-3"><div className="grid grid-cols-4 gap-2 text-xs">{ASPECTS.map((aspect) => <button key={aspect} onClick={() => setProject((p) => ({ ...p, aspect }))} className={`min-h-12 rounded-2xl px-2 py-2 font-black ring-1 active:scale-95 ${project.aspect === aspect ? 'bg-white text-slate-950 ring-white' : 'bg-white/[0.08] text-white ring-white/10'}`}>{aspect}</button>)}</div><div className="grid grid-cols-2 gap-2 text-xs"><button onClick={() => setProject((p) => ({ ...p, aspect: '9:16' }))} className="rounded-2xl bg-white/[0.08] px-3 py-3 font-bold ring-1 ring-white/10">TikTok/Reels</button><button onClick={() => setProject((p) => ({ ...p, aspect: '1:1' }))} className="rounded-2xl bg-white/[0.08] px-3 py-3 font-bold ring-1 ring-white/10">Post square</button></div></div>}

            {tool === 'export' && <div className="space-y-3"><div className="grid grid-cols-4 gap-2 text-xs">{ASPECTS.map((aspect) => <button key={aspect} onClick={() => setProject((p) => ({ ...p, aspect }))} className={`min-h-11 rounded-2xl px-2 py-2 font-black ring-1 active:scale-95 ${project.aspect === aspect ? 'bg-white text-slate-950 ring-white' : 'bg-white/[0.08] text-white ring-white/10'}`}>{aspect}</button>)}</div><button onClick={exportVideo} disabled={exporting} className="flex min-h-[3.25rem] w-full items-center justify-center gap-2 rounded-[22px] bg-white px-4 py-3 font-black text-slate-950 shadow-[0_15px_40px_rgba(255,255,255,0.14)] disabled:opacity-60">{exporting ? <Loader2 className="h-5 w-5 animate-spin" /> : <Download className="h-5 w-5" />} {exporting ? `Rendering ${Math.round(exportProgress)}%` : 'Render mobile video'}</button>{exportUrl && <button onClick={downloadExport} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-emerald-500 px-4 py-3 font-black text-emerald-950"><Check className="h-5 w-5" /> Download render</button>}{exportError && <p className="rounded-2xl bg-red-500/10 p-3 text-xs text-red-100 ring-1 ring-red-300/20">{exportError}</p>}</div>}
          </section>

          <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-[#050510]/90 pb-safe backdrop-blur-2xl">
            <div className="mx-auto max-w-[430px] px-3 pt-2"><div className="no-scrollbar flex gap-2 overflow-x-auto pb-2">{TOOLS.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => setTool(id)} className={`flex min-h-[4rem] min-w-[4.85rem] flex-col items-center justify-center gap-1 rounded-[22px] px-3 py-2 text-[11px] font-black ring-1 active:scale-95 ${tool === id ? 'bg-white text-slate-950 ring-white shadow-[0_12px_35px_rgba(255,255,255,0.16)]' : 'bg-white/[0.07] text-white/60 ring-white/10'}`}><Icon className="h-5 w-5" />{label}</button>)}</div></div>
          </nav>
        </main>
      )}
